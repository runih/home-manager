...
- Host `flake.nix` files call `mkHome`, composing modules picked from the shared registry (`m.<name>`) with any host-specific `.nix` files that live next to it (e.g. `hosts/linux/macnix/home.nix`, `hyprland.nix`, `waybar.nix`, `theme-switcher.nix`, `theme-lib.nix`, `themes.nix`; `hosts/mac/iMac/home.nix` — python3 + sourcing the multi-user nix-daemon profile script in zsh, imported by path, not through the registry, since nothing else needs it). `hosts/linux/nixos`, `nixos2`, and `madakara-nixos` are near-identical (separate physical/virtual Linux boxes), and are kept as independent host flakes rather than parameterized into one.

`hosts/linux/macnix/nixos/` is different from every other file next to it: it's a **verbatim mirror of that machine's `/etc/nixos`** (system-level NixOS config — bootloader, kernel modules, services, users, the custom out-of-tree `macbook12-audio.nix` speaker driver), not home-manager modules, and it is never imported by this flake or `mkHome`. `/etc/nixos` on macnix has no git remote, so its real files live here instead and get backed up with everything else. It has its own `flake.nix`/`flake.lock` pinning `nixpkgs` to `nixos-26.05` — the same release home-manager pins — so system packages and home-manager packages stay on matching library versions (a stray unpinned system config previously drifted onto rolling `nixos-unstable`, causing `fontconfig` schema-mismatch warnings during `hm` because home-manager's older pinned fontconfig couldn't parse config generated by the system's newer one). It also carries a `nixpkgs-unstable` input used **only** for Hyprland: `programs.hyprland.package` / `portalPackage` in `nixos/programs.nix` are set to `pkgs-unstable.hyprland` / `pkgs-unstable.xdg-desktop-portal-hyprland` so macnix runs the latest Hyprland release (0.56.2) instead of the 0.55.4 that `nixos-26.05` ships. Keep this `nixpkgs-unstable` pin in step with the root flake's (the HM side — `hyprland.nix`, `omarchy4/`, `omarchy-session.nix` — builds the matching `pkgsUnstable.hyprland` + `hyprlandPlugins.hyprexpo`).
...
`hosts/linux/nixos-pi5/nixos/` is the same arrangement for the Raspberry Pi 5 (`nixos-pi5`): a verbatim mirror of that machine's `/etc/nixos` (`kea` DHCP, docker, the out-of-tree `raspberry-pi5-leds.nix` LED-monitor build, btrfs snapshot scripts in `security.nix`), never imported by `mkHome`, with its own `flake.nix`/`flake.lock` pinning `nixpkgs` to `nixos-26.05`. Its `configuration.nix` keeps `system.stateVersion = "25.11"` (a compat marker — leave it) even though the flake now tracks `nixos-26.05`; the machine was on `nixos-25.05` before this migration.

The kernel comes from `nixos-hardware.nixosModules.raspberry-pi-5` (added as a flake input, `nixpkgs.follows`), **not** `pkgs.linuxPackages_rpi4` — nixpkgs deprecated the `linux-rpi` series. The module supplies the Raspberry Pi vendor fork (`linux-rpi` pname) at a much newer tag (6.18.x as of this migration, up from 6.12). Do **not** re-add `boot.kernelPackages = pkgs.linuxPackages_rpi4` to `hardware-configuration.nix` — it overrides the module's `mkDefault` and pins the kernel back at 6.12. `boot.nix` sets `hardware.deviceTree.filter = lib.mkForce null` to undo the module's `"bcm2712*-rpi-*.dtb"` filter, which otherwise makes NixOS's `filterDTBs` drop `dtbs/overlays/` — `tryboot.nix` copies that dir and the tryboot template loads `dtoverlay=vc4-kms-v3d-pi5` from it. `hardware.raspberry-pi.firmware.enable` stays `false` (its default) so nothing but `tryboot.nix` ever writes `/boot`.
...
- `platform/` — `basic-linux.nix` / `basic-mac.nix` (core package set + `home.username`/`homeDirectory`/`stateVersion`), `allowUnfree.nix`.
...
- `desktop/` — `niri.nix` (Wayland compositor config) and `nerd-fonts.nix`. `hosts/linux/macnix` additionally has its own Hyprland setup (`hyprland.nix` + `waybar.nix` + theme files) living next to its host flake rather than in `modules/`, and currently ships both `m.niri` and the host's Hyprland config side by side.
  ...
  Separately, `enableOmarchy4Session` in `hosts/linux/macnix/flake.nix` (independent of `enableOmarchy`) adds an **experimental "Omarchy 4" GDM session** that runs the *real* upstream [`github:omacom/omarchy`](https://github.com/omacom/omarchy) tree (Quattro — one Quickshell process, not omarchy-nix's older waybar/walker/mako stack) isolated in `~/.config-omarchy4`, next to the normal Hyprland session. The `omarchy4` input (`flake = false`) is threaded from the root flake; `hosts/linux/macnix/omarchy4/` is a folder split by concern — `tree.nix` shebang-patches the checkout's ~700 `#!/bin/bash` / `#!/usr/bin/python3` scripts for NixOS (plus macnix-local patches) and points `$OMARCHY_PATH` at the result, `runtime-deps.nix` lists the nixpkgs-available deps, `launcher.nix` drops the fixed-path launcher, `nvim.nix` builds the LazyVim config, `default.nix` wires it all; `hosts/linux/macnix/nixos/omarchy4/` registers the GDM entry. Best case it gets the compositor + Quickshell shell + in-repo theming; every menu action that shells out to pacman/yay/snapper/limine is dead by design, and Hyprland/Quickshell API skew is the main thing to iterate on — Hyprland is `pkgsUnstable` (0.56.2, matching the rest of macnix) but Quickshell is still nixos-26.05's 0.3.0. Neovim is the one Arch package (`omarchy-nvim`/`omarchy-lazyvim`) rebuilt from parts: `nvim.nix` assembles LazyVim starter + omarchy's overlay, isolated with `NVIM_APPNAME=omarchy-nvim`, and lets `lazy.nvim` bootstrap plugins on first launch (the user's real `~/.config/nvim` is untouched). See `hosts/linux/macnix/omarchy4/README.md`.
...
- `nixpkgs`/`home-manager` are pinned to a specific release branch (currently `nixos-26.05` / `release-26.05`) across root and host flakes — keep these in sync when bumping. `hosts/linux/macnix/nixos/flake.lock` additionally pins `nixpkgs-unstable` (for Hyprland only) — keep its rev matching the root flake's `nixpkgs-unstable` so the system compositor and the home-manager-built `hyprland.nix` plugin stay on the same Hyprland version.
