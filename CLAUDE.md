...
`hosts/linux/macnix/nixos/` is different from every other file next to it: it's a **verbatim mirror of that machine's `/etc/nixos`** (system-level NixOS config — bootloader, kernel modules, services, users, the custom out-of-tree `macbook12-audio.nix` speaker driver), not home-manager modules, and it is never imported by this flake or `mkHome`. `/etc/nixos` on macnix has no git remote, so its real files live here instead and get backed up with everything else; `/etc/nixos/configuration.nix` on that machine is just a one-line shim (`import /home/runih/.config/home-manager/hosts/linux/macnix/nixos/configuration.nix`). Applied with `sudo nixos-rebuild switch` (not `home-manager switch`) directly on macnix — edit the files here, they take effect on that machine's next rebuild without any copying step, since the shim imports this path live.
