# macnix desktop bits that belong to the hand-rolled Hyprland stack
# (hyprland.nix + waybar.nix + theme-switcher.nix): the hyprlock screen,
# the wofi launcher styling, the wallpaper / status helper scripts and the
# timers that feed the lock screen.
#
# Split out of home.nix so the Omarchy toggle in ./flake.nix can drop all
# of it in one move — omarchy-nix manages its own hyprlock / wofi / waybar
