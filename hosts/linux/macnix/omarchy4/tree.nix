...
-- Pin these devices back to se(mac)-equivalent and drop the Apple-specific
-- kb_model/options, which don't apply to this keyboard's physical layout
-- either. kb_options intentionally omits compose:ralt (it stole the
-- physical AltGr key needed for $, #, etc. on se(mac)).
--
-- Uses the dedicated macnix-se-pok3r layout (hosts/linux/macnix/nixos/
-- custom_mac_se_pok3r) rather than stock se(mac) directly: plain se(mac)
-- maps the ´/` key to non-combining acute/grave instead of dead_acute/
-- dead_grave, silently breaking composed accents (´ + u -> ú comes out as
-- literal "´u"). That file restores just the dead keys, without the
-- internal-keyboard-only LSGT/TLDE swap that macnix-se's "basic" variant
-- carries.
--
-- This is its own top-level layout, NOT a macnix-se variant (e.g.
-- macnix-se(pok3r)) — a custom variant added on top of a stock layout
-- silently failed to apply via kb_layout/kb_variant here (stuck on the
-- old value even across a full Hyprland restart, despite the exact RMLVO
-- tuple compiling fine standalone via `xkbcli compile-keymap`). Follow
-- the Hyprland wiki's own custom-layout convention instead: a dedicated
